...
#' 图形事件 onstackbarClick: function  条形点击事件
#' onstackbarDblClick: function  条形双击事件
#' onstackbarMousemove: function  条形鼠标移动事件
#' onstackbarContextmenu: function    条形右键事件
#' 其他事件类型见通用图表配置。
#' @examples 
...
#' stackbar(data,xField='region',yField='sales')
...
#' Shiny bindings for stackbar
#'
#' Output and render functions for using stackbar within Shiny
...
#' @param expr An expression that generates a stackbar
...
#'
#' @export
stackBarOutput <- function(outputId, width = '100%', height = '400px') {
  htmlwidgets::shinyWidgetOutput(outputId, 'stackbar', width, height, package = 'g2plot')
}

#' @rdname stackbar-shiny
#' @export
renderStackBar <- function(expr, env = parent.frame(), quoted = FALSE) {
  if (!quoted) { expr <- substitute(expr) }
  #' force quoted
  htmlwidgets::shinyRenderWidget(expr, stackbarOutput, env, quoted = TRUE)
}
