...
# stroke: string  折线颜色
# bubbleWidth: number  折线宽度
# bubbleDash: number[] 虚线显示
# opacity: number  透明度
...
#' Shiny bindings for bubble
#'
#' Output and render functions for using bubble within Shiny
...
#' @param expr An expression that generates a bubble
...
#'
#' @export
bubbleOutput <- function(outputId, width = '100%', height = '400px') {
  htmlwidgets::shinyWidgetOutput(outputId, 'bubble', width, height, package = 'g2plot')
}

#' @rdname bubble-shiny
#' @export
renderBubble <- function(expr, env = parent.frame(), quoted = FALSE) {
  if (!quoted) { expr <- substitute(expr) }
  # force quoted
  htmlwidgets::shinyRenderWidget(expr, bubbleOutput, env, quoted = TRUE)
}
